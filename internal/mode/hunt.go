...
// huntMode is vala's default specialization and the one the harness has always
// run: the full eight-stage hunt loop that turns a hypothesis into a durable
// detection or a documented coverage decision. Its tool policy is nil (expose
// every registered tool) and it bundles no skills, so it reproduces the classic
// behavior exactly.
func huntMode() Mode {
	return Mode{
		ID:          "hunt",
		Title:       "Threat Hunting",
		Description: "The full eight-stage hunt loop: hypothesis → evidence → detection.",
		Intro:       huntIntro,
		PromptBody:  func(PromptInput) string { return huntBody },
		ToolPolicy:  nil, // expose every tool
	}
}

const huntIntro = `This is vala, an agentic threat-hunting system.
...
const huntBody = `# The hunt loop
This is hunt mode; within it, everything is a tool — compose primitives freely.
The brain stores backlog items, hunts, intel, evidence, detections, and coverage
as connected, first-class artifacts; pick the smallest set of tools and link
related artifacts together. The loop has eight stages; stages 4–6 iterate as
evidence builds:
